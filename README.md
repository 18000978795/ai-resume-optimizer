...
- [ ] **接入真实 AI API**：替换 `MockAiProvider`，接入 Claude/OpenAI 生成更精准的优化建议
- [ ] **PDF 简历导出**：生成可直接投递的 PDF 格式简历
- [ ] **多份简历管理**：用户账号体系，管理多份不同岗位的简历
- [ ] **简历对比**：优化前后对照视图，直观展示改进
- [ ] **岗位匹配度**：基于 JD 文本分析，输出岗位匹配度评分
- [ ] **国际化**：支持英文简历生成
- [ ] **单元测试**：前后端 Jest/Vitest 测试覆盖
